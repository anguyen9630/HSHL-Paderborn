...
/*_______________Scheduler class______________*/
...
void TrafficScheduler::fReserveBlocks(int p_Road, int p_Direction)
{
  if (p_Road == Rd_East)
  {
    if (p_Direction == Straight)
    {
      ...
      this->v_Block_NECounter = millis();
      this->v_Block_NEDirection = p_Road;
      this->v_Block_NWCounter = millis();
      this->v_Block_NWDirection = p_Road;      
    }
    ...
    else if (p_Direction == TurnLeft)
    {
      ...
      this->v_Block_NWCounter = millis();
      this->v_Block_NWDirection = p_Road;
      this->v_Block_SWCounter = millis();
      this->v_Block_SWDirection = p_Road;
    }
  }
  else if (p_Road == Rd_West)
  {
    if (p_Direction == Straight)
    {
      ...
      this->v_Block_SWCounter = millis();
      this->v_Block_SWDirection = p_Road;
      this->v_Block_SECounter = millis();
      this->v_Block_SEDirection = p_Road;      
    }
    else if (p_Direction == TurnRight)
    {
      //Reserve BlockSW
...
std::vector<Task> TrafficScheduler::fReleaseVehicle()
{
  ...
  if (v_VehiclesCanBeReleased.size() > 1)
  {
    ...
    for (int v_indx = 1; v_indx < v_VehicleToRelease.size(); v_indx++)
    {
  ...
  if (v_VehiclesCanBeReleased.size())
  {
    // Reserve the blocks
    ...
    for (int v_indx = 0; v_indx < v_VehicleToRelease.size(); v_indx++)
    {
      switch (v_VehicleToRelease[v_indx].fRtRoad())
      {
      case Rd_North:
